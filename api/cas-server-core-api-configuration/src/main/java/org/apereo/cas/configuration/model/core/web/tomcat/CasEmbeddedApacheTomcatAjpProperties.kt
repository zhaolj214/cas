...
/**
 * This is [CasEmbeddedApacheTomcatAjpProperties].
 *
 * @author Misagh Moayyed
 * @since 5.3.0
 */
@RequiresModule(name = "cas-server-webapp-tomcat")
class CasEmbeddedApacheTomcatAjpProperties : Serializable {
    /**
     ...
     */
    var isAllowTrace: Boolean = false
     ...
     */
    var isEnableLookups: Boolean = false
    /**
     ...
     * Examples include:
     *
     *  * `tomcatAuthentication`: If set to true, the authentication will be done in Tomcat.
     * Otherwise, the authenticated principal will be propagated from the native webserver
     * and used for authorization in Tomcat.
     * Note that this principal will have no roles associated with it. The default value is true.
     ...
     * ignored as the connector will execute tasks using the executor rather than an internal thread pool.
     *  * `keepAliveTimeout`: The number of milliseconds this Connector
     ...
     * A value of less than zero means no limit. If not specified, a default value of 200 will be used.
     *  * `bufferSize`: The size of the output buffer to use. If less than or equal to zero,
     * then output buffering is disabled. The default value is -1 (i.e. buffering disabled)
     *  * `clientCertProvider`: When client certificate information is presented in a
     ...
     *  * `connectionTimeout`: The number of milliseconds this Connector
     * will wait, after accepting a connection,
     * for the request URI line to be presented. The default value is infinite (i.e. no timeout).
     ...
     * A value of 127.0.0.1 indicates that the Connector will only listen on the loopback interface.
     *
     *
     *

package org.apereo.cas.configuration.model.core.authentication

import org.apereo.cas.configuration.support.RequiresModule

import java.io.Serializable

/**
 * This is [PasswordEncoderProperties].
 *
 * @author Misagh Moayyed
 * @since 5.0.0
 */
@RequiresModule(name = "cas-server-core-authentication", automated = true)
class PasswordEncoderProperties : Serializable {

    /**
     * Define the password encoder type to use.
     * Type may be specified as blank or 'NONE' to disable password encoding.
     * It may also refer to a fully-qualified class name that implements
     * the [org.springframework.security.crypto.password.PasswordEncoder] interface
     * if you wish you define your own encoder.
     * @see PasswordEncoderTypes
     */
    var type = "NONE"

    /**
     * The encoding algorithm to use such as 'MD5'.
     * Relevant when the type used is 'DEFAULT'.
     */
    var encodingAlgorithm: String? = null

    /**
     * The encoding algorithm to use such as 'UTF-8'.
     * Relevant when the type used is 'DEFAULT'.
     */
    var characterEncoding: String? = null

    /**
     * Secret to use with STANDARD, PBKDF2, BCRYPT password encoders.
     * Secret usually is an optional setting.
     */
    var secret: String? = null

    /**
     * Strength or number of iterations to use for password hashing.
     * Usually relevant when dealing with PBKDF2 or BCRYPT encoders.
     */
    var strength = 16

    enum class PasswordEncoderTypes {
        /**
         * No password encoding will take place.
         */
        NONE,
        /**
         * Uses an encoding algorithm and a char encoding algorithm.
         */
        DEFAULT,
        /**
         * Uses [org.springframework.security.crypto.password.StandardPasswordEncoder].
         * A standard `PasswordEncoder` implementation that uses SHA-256 hashing with 1024
         * iterations and a random 8-byte random salt value.
         */
        STANDARD,
        /**
         * Uses [org.springframework.security.crypto.bcrypt.BCryptPasswordEncoder].
         */
        BCRYPT,
        /**
         * Uses [org.springframework.security.crypto.scrypt.SCryptPasswordEncoder].
         */
        SCRYPT,
        /**
         * Uses [org.springframework.security.crypto.password.Pbkdf2PasswordEncoder].
         */
        PBKDF2
    }

    companion object {
        private const val serialVersionUID = -2396781005262069816L
    }
}


